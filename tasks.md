...
**Status:** done
**Branch:** agent/r14-005-phase1-2026-04-20
...
- [x] Read `v1.4_SPEC.md` §7, §8, §9 first
- [x] Implement `rapid2 v1.4/capital.py` per §7 — constants at top, dataclass, `compute_state` function
- [x] Implement `rapid2 v1.4/strategy.py` per §8 — thin orchestrator, calls `core.dca.evaluate_dca` for each core symbol and `agents.mean_reversion.evaluate` when satellite enabled
- [x] In strategy.py, add a helper to determine `btc_above_200ma` from a fetch_ohlcv call — one call cached for the decision
- [x] In bot.py, replace the 5 placeholder Telegram handlers with real ones that call `capital.compute_state` and use the last stored `OrchestratorDecision`
- [x] Write `tests/test_capital.py` — 5 cases from §7
- [x] Write `tests/test_strategy.py` — 4 cases from §8, mock fetch_ohlcv for the 200MA check
